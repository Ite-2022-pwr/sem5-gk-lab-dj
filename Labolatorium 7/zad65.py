...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void) {
            vec2  inputs = vec2(gl_InstanceID, gl_VertexID);
            float rand   = random( inputs );
            vec4  pos   = vec4( rand/15.0 );
        
            gl_Position = P_matrix * V_matrix * M_matrix * (position + pos + vec4(gl_InstanceID%10, gl_InstanceID/10, 0.0, 0.0));
            vertex_color = color;
        }
    """
...
def startup():
    ...
    glBufferData(GL_ARRAY_BUFFER, vertex_color, GL_STATIC_DRAW)
